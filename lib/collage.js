...
async function generateCollage(db, gameId, upRoot, onProgress = () => {}) {
  ...
  // than XFADE (below) or the transition would consume the whole clip.
  const W = 1280, H = 720, FPS = 30;
  const PHOTO_SECS = 2.5;
  const TITLE_SECS = PHOTO_SECS * 2; // cover/selfie card holds 2x a normal photo
  ...
  const XFADE = 1.0; // seconds of crossfade between consecutive clips
  const segDur = steps.map(s => (s.kind === 'title' ? TITLE_SECS : PHOTO_SECS));

  if (segmentFiles.length === 1) {
    // Nothing to fade between — just finalize the single clip.
    await new Promise((resolve, reject) => {
      ffmpeg().input(segmentFiles[0])
