...
async function generateCollage(db, gameId, upRoot, onProgress = () => {}) {
  ...
  async function worker() {
    while (true) {
      ...
      try {
        if (s.kind === 'title') {
          await renderTitleCardClip(s.team.team_name, s.team.selfie_abs, TITLE_FRAMES, W, H, FPS, segmentFiles[i]);
        } else {
          await renderPhotoClip(s.file.abs, PHOTO_FRAMES, W, H, FPS, segmentFiles[i]);
        }
      } catch (e) {
        failure = failure || e;
        return;
...
function renderTitleCardClip(teamName, selfieAbs, frames, W, H, FPS, out) {
  ...
  return renderPhotoClip(png, frames, W, H, FPS, out);
}

// A still-image clip: pad/letterbox to 1280×720, held for an exact `frames`
// count (see the note on renderTitleCardClip for why frames, not seconds).
function renderPhotoClip(abs, frames, W, H, FPS, out) {
  return new Promise((resolve, reject) => {
    ffmpeg()
      .input(abs)
      .inputOptions(['-loop', '1'])
