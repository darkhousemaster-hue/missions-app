...
async function generateCollage(db, gameId, upRoot, onProgress = () => {}) {
  ...
  await new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .inputFormat('concat')
      .inputOptions(['-safe', '0'])
      // Every segment was already encoded with identical codec/pix_fmt/fps, so
      // we can stream-copy them into the container instead of re-encoding the
      // whole timeline again. This makes the concat near-instant even for long
      // slideshows. +faststart moves the moov atom up for instant playback.
...
const INTER_PATH = path.resolve(__dirname, '..', 'assets', 'fonts', 'Inter-Bold.woff');
let _interFont; // lazy, cached
function interFont() {
  if (_interFont !== undefined) return _interFont;
  try {
    const b = fs.readFileSync(INTER_PATH);
    _interFont = opentype.parse(b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength));
  } catch (e) {
    console.warn('collage: could not load Inter font, titles will have no text:', e.message);
    _interFont = null;
  }
  return _interFont;
}
...
function sanitizeName(teamName) {
  ...
  return String(teamName || '').replace(/[ -]/g, '').trim().slice(0, 28) || 'Team';
}
